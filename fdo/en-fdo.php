...
class EnUpsfreightFdo
{
    ...
    public function en_cart_package($package)
    {
        ...
        $this->en_fdo_meta_data['plugin_name'] = 'ups_freight_quotes';
        $accessorials['residential'] = get_option('ups_freight_setting_residential') == 'yes' ? true : false;
        $accessorials['liftgate'] = get_option('ups_freight_settings_liftgate') == 'yes' ? true : false;
    ...
    public function en_package_items($items)
    {
        ...
        foreach ($items as $item_key => $item_data) {
            $productId = $productName = $productQty = $actualProductPrice = $products = $productPrice = $productWeight = $productLength = $productWidth = $productHeight = $ptype = $hazardousMaterial = $productType = $productSku = $productClass = $attributes = $variantId = $nestedMaterial = $nestedPercentage = $nestedDimension = $nestedItems = $stakingProperty = $nmfc_number = $markup = '';
            $attributes = [];
            extract($item_data);
