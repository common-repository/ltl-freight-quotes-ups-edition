...
/**
 * UPS Freight Class ups_freight_init
...
/**
 * UPS FREIGHT Class Initialize
 */
function ups_freight_init()
{
    if (!class_exists('UPS_Freight_Shipping_Class')) {

        /**
         * UPS FREIGHT Shipping Calculation Class
         */
        class UPS_Freight_Shipping_Class extends WC_Shipping_Method
        {

            public $forceAllowShipMethodUps = array();
            public $getPkgObjUps;
            public $Ups_Freight_Liftgate_As_Option;
            ...
            public function __construct($instance_id = 0)
            {
                ...
                $this->method_title = __('TForce Freight');
                $this->method_description = __('Shipping rates from TForce Freight.');
                $this->supports = array(
                    'shipping-zones',
                    'instance-settings',
                ...
                $this->enabled = "yes";
                $this->title = 'LTL Freight Quotes - TForce Edition';
                $this->init();
                $this->Ups_Freight_Liftgate_As_Option = new Ups_Freight_Liftgate_As_Option();
            }
            ...
            /**
             * Enable WooCommerce Shipping For UPS FREIGHT
             */
            function init_form_fields()
            {
                $this->instance_form_fields = array(
                    'enabled' => array(
                        'title' => __('Enable / Disable', 'ups_freight'),
                        'type' => 'checkbox',
                        'label' => __('Enable This Shipping Service', 'ups_freight'),
                        'default' => 'no',
                        'id' => 'ups_freight_enable_disable_shipping'
                    )
                );
            }
             ...
             * @return type
             */
            public function forceAllowShipMethodUps($forceShowMethods)
            {
                if (!empty($this->getPkgObjUps->ValidShipmentsArr) && (!in_array("ltl_freight", $this->getPkgObjUps->ValidShipmentsArr))) {
                    $this->forceAllowShipMethodUps[] = "free_shipping";
                    $this->forceAllowShipMethodUps[] = "valid_third_party";
                } else {

                    $this->forceAllowShipMethodUps[] = "ltl_shipment";
                }

                $forceShowMethods = array_merge($forceShowMethods, $this->forceAllowShipMethodUps);
            ...
            public function en_virtual_products()
            {
                ...
                if (!empty($items)) {
                    ...
                    $virtual_fdo = [
                        'plugin_type' => 'ltl',
                        'plugin_name' => 'ups_freight_quotes',
                        'accessorials' => '',
                        'items' => $items,
                        'address' => '',
            ...
            /**
             * Calculate Shipping Rates For UPS FREIGHT
            ...
            public function calculate_shipping($package = array(), $eniture_admin_order_action = false)
            {
                ...
                $srate = array();
                $rate = array();
                $this->package_plugin = get_option('ups_freight_package');
                $ups_woo_obj = new UPS_Freight_Woo_Update_Changes();
                $freight_zipcode = (strlen(WC()->customer->get_shipping_postcode()) > 0) ? WC()->customer->get_shipping_postcode() : $ups_woo_obj->ups_freight_postcode();

                $coupn = WC()->cart->get_coupons();
                if (isset($coupn) && !empty($coupn)) {
                    $freeShipping = $this->upsLTLFreeShipping($coupn);
                    if ($freeShipping == 'y')
                        return FALSE;
                }

                $get_package = new UPS_Freight_Shipping_Get_Package();
                $this->getPkgObjUps = $get_package;

                $ups_freight_res_inst = new UPS_Freight_Get_Shipping_Quotes();

                $this->web_service_inst = $ups_freight_res_inst;

                $this->ups_freight_ltl_shipping_quote_settings();

                $ups_freight_package = $get_package->group_ups_freight_shipment($package, $ups_freight_res_inst, $freight_zipcode);
                $handling_fee = get_option('ups_freight_settings_handling_fee');
                $quotes = array();
                ...
                add_filter('force_show_methods', array($this, 'forceAllowShipMethodUps'));

                $no_param_multi_ship = 0;
                if (@count($ups_freight_package) > 1) {
                    foreach ($ups_freight_package as $key => $value) {
                        if (isset($value["NOPARAM"]) && $value["NOPARAM"] === 1 && empty($value["items"])) {
                            $no_param_multi_ship = 1;
                            unset($ups_freight_package[$key]);
                        }
                    }
                }
                ...
                if (isset($ups_freight_package) && !empty($ups_freight_package)) {
                    $ltl_products = $small_products = [];
                    foreach ($ups_freight_package as $locId => $sPackage) {
                        if (array_key_exists('upsfreight', $sPackage)) {
                            $ltl_products[] = $sPackage;
                            $web_service_arr = $ups_freight_res_inst->ups_freight_shipping_array($sPackage, $this->package_plugin);
                            $response = $ups_freight_res_inst->ups_freight_get_web_quotes($web_service_arr);
                            $this->InstorPickupLocalDelivery = $ups_freight_res_inst->return_localdelivery_array();
                            if (empty($response) && empty($this->InstorPickupLocalDelivery)) {
                                return [];
                            }
                            $quotes[] = $response;
                            continue;
                ...
                if (count($quotes) > 1 || $smpkgCost > 0 || !empty($virtual_rate)) {
                    ...
                    (isset($small_quotes) && count($small_quotes) > 0) ? $this->minPrices['EN_UPS_LIFT'] = $small_quotes : "";
                    (isset($small_quotes) && count($small_quotes) > 0) ? $this->minPrices['EN_UPS_NOTLIFT'] = $small_quotes : "";
                    (isset($small_quotes) && count($small_quotes) > 0) ? $this->minPrices['EN_UPS_HAT'] = $small_quotes : "";
                    ...
                    if (!empty($virtual_rate)) {
                        ...
                        $this->minPrices['EN_UPS_LIFT'] = isset($this->minPrices['EN_UPS_LIFT']) && !empty($this->minPrices['EN_UPS_LIFT']) ? array_merge($this->minPrices['EN_UPS_LIFT'], $virtual_meta_rate) : $virtual_meta_rate;
                        $this->minPrices['EN_UPS_NOTLIFT'] = isset($this->minPrices['EN_UPS_NOTLIFT']) && !empty($this->minPrices['EN_UPS_NOTLIFT']) ? array_merge($this->minPrices['EN_UPS_NOTLIFT'], $virtual_meta_rate) : $virtual_meta_rate;
                    ...
                    foreach ($quotes as $key => $quote) {
                        if (!empty($quote)) {
                            ...
                            $this->minPrices['EN_UPS_LIFT'][$key] = $rates;

                            // FDO
                            $this->en_fdo_meta_data['EN_UPS_LIFT'][$key] = (isset($rates['meta_data']['en_fdo_meta_data'])) ? $rates['meta_data']['en_fdo_meta_data'] : [];
                            ...
                            if (isset($this->quote_settings['liftgate_delivery_option']) &&
                                ...
                                $this->minPrices['EN_UPS_NOTLIFT'][$key] = $s_rates;

                                $this->en_fdo_meta_data['EN_UPS_NOTLIFT'][$key] = (isset($s_rates['meta_data']['en_fdo_meta_data'])) ? $s_rates['meta_data']['en_fdo_meta_data'] : [];
                                ...
                                if(is_array($s_rates['product_level_markup_arr']) && count($s_rates['product_level_markup_arr']) > 0){
                                    $s_cost = $this->add_product_level_markup($s_cost, $s_rates['product_level_markup_arr']);
                                }
                                ...
                                $this->minPrices['EN_UPS_NOTLIFT'][$key]['cost'] = $this->add_handling_fee($s_cost, $handling_fee);
                                $this->en_fdo_meta_data['EN_UPS_NOTLIFT'][$key]['rate']['cost'] = $this->add_handling_fee($s_cost, $handling_fee);
                            }

                            // product level markup
                            if(is_array($rates['product_level_markup_arr']) && count($rates['product_level_markup_arr']) > 0){
                                $_cost = $this->add_product_level_markup($_cost, $rates['product_level_markup_arr']);
                            }
                            ...
                            $this->minPrices['EN_UPS_LIFT'][$key]['cost'] = $this->add_handling_fee($_cost, $handling_fee);
                            $this->en_fdo_meta_data['EN_UPS_LIFT'][$key]['rate']['cost'] = $this->add_handling_fee($_cost, $handling_fee);
                    ...
                    ($s_multi_cost > 0) ? $rate[] = $this->arrange_multiship_freight(($s_multi_cost + $smpkgCost), 'EN_UPS_NOTLIFT', $s_label, $s_append_label) : "";
                    ($multi_cost > 0 || $smpkgCost > 0) ? $rate[] = $this->arrange_multiship_freight(($multi_cost + $smpkgCost), 'EN_UPS_LIFT', $_label, $append_label) : "";
                    $rates = $rate;
                    ...
                    if (!empty($quote)) {
                        ...
                        if (is_array($rates) && (!empty($rates))) {
                            foreach ($rates as $key => $quote) {
                                ...
                                // product level markup
                                if(is_array($quote['product_level_markup_arr']) && count($quote['product_level_markup_arr']) > 0){
                                    $_cost = $this->add_product_level_markup($_cost, $quote['product_level_markup_arr']);
                                }
                ...
                if (isset($rates)) {
                    $rates = $this->ups_freight_quotes_add_rate_arr($rates);
                }else{
                    $rates = [];
                }
             ...
             * @global $wpdb $wpdb
             */
            function ups_freight_ltl_shipping_quote_settings()
            {
                $this->web_service_inst->quote_settings['label_as'] = get_option('ups_freight_setting_label_as');
                $this->web_service_inst->quote_settings['handling_fee'] = get_option('ups_freight_settings_handling_fee');
                $this->web_service_inst->quote_settings['liftgate_delivery'] = get_option('ups_freight_settings_liftgate');
                $this->web_service_inst->quote_settings['liftgate_delivery_option'] = get_option('ups_freight_quotes_liftgate_delivery_as_option');
                $this->web_service_inst->quote_settings['residential_delivery'] = get_option('ups_freight_setting_residential');
                ...
                // Cuttoff Time
                $this->web_service_inst->quote_settings['delivery_estimates'] = get_option('ups_delivery_estimates');
                $this->web_service_inst->quote_settings['orderCutoffTime'] = get_option('ups_freight_order_cut_off_time');
                $this->web_service_inst->quote_settings['shipmentOffsetDays'] = get_option('ups_freight_shipment_offset_days');
                $this->web_service_inst->quote_settings['handling_weight'] = get_option('ups_freight_settings_handling_weight');
                $this->web_service_inst->quote_settings['maximum_handling_weight'] = get_option('ups_freight_maximum_handling_weight');
            }
            ...
            /**
             * List quotes woocommerce
             * @param array type $add_rate_arr
             */
            public function ups_freight_quotes_add_rate_arr($add_rate_arr)
            {
                if (isset($add_rate_arr) && (!empty($add_rate_arr)) && (is_array($add_rate_arr))) {
                    ...
                    foreach ($add_rate_arr as $key => $rate) {
                        ...
                        $rate['id'] = isset($rate['id']) && is_string($rate['id']) ? 'ups_freight:' . $rate['id'] : '';
...
//              return the rates
            ...
            function pickup_delivery($label, $en_wd_origin_array, $total_distance)
            {
                ...
                $pickup_delivery = array(
                    'id' => 'ups_freight:' . 'in-store-pick-up',
                    'cost' => 0,
                    'label' => $label,
                    'plugin_name' => 'upsLtl',
                    'plugin_type' => 'ltl',
            ...
            public function set_label_in_quote($rate)
            {
                ...
                $rate_label = ($this->quote_settings['shipment'] == "single_shipment") ? $this->ups_freight_label_as() : 'Freight';

                $rate_label .= $this->ups_freight_filter_from_label_sufex($label_sufex);
            ...
            public function ups_freight_filter_from_label_sufex($label_sufex)
            {
                $append_label = "";
                $rad_status = true;
             ...
             * @return string type
             */
            public function ups_freight_label_as()
            {
             ...
             * @return string
             */
            function upsLTLFreeShipping($coupon)
            {
            ...
            function add_product_level_markup($cost, $product_level_markup_arr){
                ...
                foreach($product_level_markup_arr as $pro_id => $markup){
                    $cost = $this->add_handling_fee($cost, $markup);
                }
